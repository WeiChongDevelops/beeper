...
exports.onContactChanged = void 0;
...
const handler_1 = require("./ai/handler");
const language_helper_1 = require("./language-helper");
const mac_contacts_helper_1 = require("./mac-contacts-helper");
require("./oauth");
const media_request_handler_1 = require("./media-request-handler");
const oauth_1 = require("./oauth");
const protocol_1 = require("./protocol");
const rageshake_1 = require("./rageshake");
...
const macRegistrationProvider = __importStar(require("./mac-registration-provider"));
...
if (userDataPathInProtocol) {
    electron_1.app.setPath("userData", userDataPathInProtocol);
}
else if (argv["profile-dir"]) {
    electron_1.app.setPath("userData", argv["profile-dir"]);
}
else if (argv["profile"]) {
    electron_1.app.setPath("userData", `${electron_1.app.getPath("userData")}-${argv["profile"]}`);
}
...
async function setupGlobals() {
    ...
    iconPath = path_1.default.join(iconsPath, iconSubdir, iconFile);
    // Unused on non win32 platforms
    const unreadIconPath = path_1.default.join(iconsPath, iconSubdir, unreadIcon);
    trayConfig = {
        icon_path: iconPath,
    ...
    // launcher
    launcher = new auto_launch_1.default({
...
const store = new electron_store_1.default({ name: "electron-config" });
let eventIndex = null;
let mainWindow = null;
global.appQuitting = false;
(0, rageshake_1.interceptConsole)(console, (key, ...args) => {
...
const warnBeforeExit = (event, input) => {
    ...
    if (shouldWarnBeforeExit && exitShortcutPressed) {
        const shouldCancelCloseRequest = electron_1.dialog.showMessageBoxSync(mainWindow, {
            type: "question",
            buttons: [(0, language_helper_1._t)("Cancel"), (0, language_helper_1._t)("Close Beeper")],
            message: (0, language_helper_1._t)("Please leave Beeper running in the background if possible"),
            defaultId: 1,
            cancelId: 0,
        }) === 0;
...
const deleteContents = async (p) => {
    for (const entry of await fs_1.promises.readdir(p)) {
...
const mediaRequestHandler = new media_request_handler_1.MediaRequestHandler();
electron_1.ipcMain.on("setMatrixAccessToken", (ev, accessToken, homeserverHost) => {
    mediaRequestHandler.saveMatrixAccessToken(accessToken, homeserverHost);
});
electron_1.ipcMain.on("setBadgeCount", function (ev, count) {
...
electron_1.ipcMain.on("setIconOverlay", function (ev, image) {
    if (process.platform == "win32") {
        iconOverlay = image != null ? electron_1.nativeImage.createFromDataURL(image) : null;
        updateIconOverlay();
    }
});
...
electron_1.ipcMain.on("app_onAction", function (ev, payload) {
...
electron_1.ipcMain.on("ipcCall", async function (ev, payload) {
    if (!mainWindow)
        return;
    try {
...
async function handleIpcCall({ name, args, id }) {
    let ret;
    switch (name) {
        case "copyToClipboard":
            electron_1.clipboard.writeText(args[0]);
            break;
        ...
        case "getAppLocaleCountryCode":
            ret = electron_1.app.getLocaleCountryCode();
            break;
        case "getAppName":
            ret = electron_1.app.getName();
            break;
        case "getOs":
            ret = os_1.default.release();
            break;
        case "getOsVersion":
            ret = child_process_1.default.execSync("sw_vers -productVersion").toString().trim();
            break;
        case "getCpuArch":
            ret = process.arch;
            break;
        ...
        case "startOAuthServer":
            ret = await (0, oauth_1.startOAuthServer)();
            break;
        case "stopOAuthServer":
            (0, oauth_1.stopOAuthServer)();
            break;
        case "contactOperation":
            ...
            */
            ...
            ret = await (0, mac_contacts_helper_1.contactsHelper)(args[0].operation, args[0].content);
            break;
        case "aiOperation":
            ret = await (0, handler_1.handleAiOperation)(args[0]);
            break;
        case "getContactsAuthStatus":
            console.log("Beeper : electron-main : getContactsAuthStatus");
            ret = (0, mac_contacts_helper_1.getContactsAuthStatus)();
            break;
        case "requestContactAccess":
            console.log("Beeper : electron-main : requestContactAccess");
            ret = await (0, mac_contacts_helper_1.requestContactAccess)();
            break;
        case "openExternal":
            await electron_1.shell.openExternal(args[0].url);
            break;
        default:
...
exports.onContactChanged = onContactChanged;
...
let authWindow;
electron_1.ipcMain.on("bridgeAuth", async function (ev, payload = {}) {
    if (!mainWindow)
        return;
    if (authWindow) {
        closeAuthWindow();
    ...
    authWindow.webContents.session.webRequest.onCompleted(function (details) {
        console.log(`[AuthWindowHttp] AuthWindow #${details.id} ${details.method} ${details.url} completed got ${details.statusCode} fromCache? ${details.fromCache} error? ${details.error}`);
    });
    authWindow.webContents.session.webRequest.onErrorOccurred(function (details) {
        console.log(`[AuthWindowHttp] AuthWindow #${details.id} ${details.method} ${details.url} failed got error ${details.error} fromCache? ${details.fromCache}`);
    });
                    ...
                    if (r.type == "childList") {
                        for (const node of Array.from(r.addedNodes)) {
                            if (node.nodeName == "IFRAME" &&
                                node.getAttribute("title") == "Sign in with Google Button") {
                                found(node);
                            }
                        }
                    }
    ...
    if (payload.cookie_keys) {
        cookieWatcher = async () => {
            ...
            const filtered = cookies.filter(({ name, domain }) => {
                if (payload.domains)
                    return payload.cookie_keys.includes(name) && payload.domains.includes(domain);
...
electron_1.ipcMain.on("closeAuthWindow", closeAuthWindow);
electron_1.ipcMain.on("seshat", async function (ev, payload) {
    if (!mainWindow)
        return;
    ...
    switch (payload.name) {
        ...
        case "initEventIndex":
            if (eventIndex === null) {
                try {
                    await fs_1.promises.mkdir(eventStorePath, { recursive: true });
        ...
        case "isRoomIndexed":
            if (eventIndex === null)
                ...
                ret = await eventIndex.isRoomIndexed(args[0]);
            break;
...
electron_1.app.commandLine.appendSwitch("enable-features", "PlatformHEVCDecoderSupport");
...
electron_1.app.on("ready", async () => {
    ...
    (0, mac_contacts_helper_1.setupContactListener)();
    if (argv["devtools"]) {
        try {
    ...
    electron_1.protocol.registerFileProtocol("nova", (request, callback) => {
    ...
    mainWindow.webContents.session.webRequest.onCompleted(function (details) {
        if (false && shouldLogUrl(details.url)) {
            console.log(`[MainWindowHttp] #${details.id} ${details.method} ${details.url} completed got ${details.statusCode} fromCache? ${details.fromCache} error? ${details.error}`);
        }
    });
    mainWindow.webContents.session.webRequest.onErrorOccurred(function (details) {
        if (false && shouldLogUrl(details.url)) {
            console.log(`[MainWindowHttp] #${details.id} ${details.method} ${details.url} failed got error ${details.error} fromCache? ${details.fromCache}`);
        }
    });
    electron_1.ipcMain.on("minimize", () => mainWindow.minimize());
    electron_1.ipcMain.on("maximize", () => mainWindow.maximize());
    ...
    (0, webcontents_handler_1.default)(mainWindow.webContents);
    appLocalization = new language_helper_1.AppLocalization({
        store,
        components: [() => tray.initApplicationMenu(), () => electron_1.Menu.setApplicationMenu((0, vectormenu_1.buildMenuTemplate)())],
    });
});
...
function beforeQuit() {
    ...
    (0, mac_contacts_helper_1.teardownContactListener)();
    if (mainWindow) {
...
electron_1.app.setAppUserModelId("Beeper");
...
electron_1.ipcMain.on("register-global-shortcuts", () => {
    console.log("Register global shortcuts");
    const ret = electron_1.globalShortcut.register("CmdOrCtrl+Shift+K", () => {
...
electron_1.ipcMain.on("unregister-global-shortcuts", () => {
...
// });
electron_1.ipcMain.handle("isFullScreen", () => mainWindow.isFullScreen());
electron_1.ipcMain.handle("isMaximized", () => mainWindow.isMaximized());
electron_1.ipcMain.handle("get-sources", async (ev, options) => {
    const sources = await electron_1.desktopCapturer.getSources(options);
