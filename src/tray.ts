...
let unreadIcon: NativeImage;

export function setUnreadStatus(unread: boolean) {
    if (trayIcon) {
...
interface IConfig {
    ...
    brand: string;
    unread_icon_path: string;
}

export function create(config: IConfig): void {
    // no trays on darwin
    if (process.platform === "darwin" || trayIcon) return;
    defaultIcon = nativeImage.createFromPath(config.icon_path);
    unreadIcon = nativeImage.createFromPath(config.unread_icon_path);
